...
it('persists activation per device and resumes automatically after vault layout readiness',async()=>{const {p,app}=make();const r=useRemote(p);await p.onload();await p.activate(cfg);await advance(1000);p.onunload();const next=make(app).p;(next as any).saved=cfg;useRemote(next,r);await next.onload();expect(next.status).toContain('等待笔记库加载');const begin=vi.spyOn(next as any,'begin');app.layout?.();await begin.mock.results[0].value;await advance(1000);expect(next.status).toContain('已同步');await next.pause();});
...
it('commands resume, pause and pull-again recover deleted local files',async()=>{const {p,app}=make();const r=useRemote(p);await seed(r);await p.onload();p.config=cfg;const commands=(p as any).commands,resume=vi.spyOn(p,'activate'),pause=vi.spyOn(p,'pause'),pull=vi.spyOn(p as any,'pullAgain');commands.find((c:any)=>c.id==='resume').callback();await resume.mock.results[0].value;await advance(1000);commands.find((c:any)=>c.id==='pause').callback();await pause.mock.results[0].value;expect(p.device.enabled).toBe(false);app.store.delete('note.md');commands.find((c:any)=>c.id==='pull-again').callback();await pull.mock.results[0].value;expect(app.store.has('note.md')).toBe(true);await p.pause();});
